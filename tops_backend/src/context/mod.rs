...
use crate::{driv::*, TopsApi};

#[derive(Debug)]
pub struct TopsContext {
    pub inner: topsCtx_t,
    pub device: topsDevice_t,
}

impl crate::memory::private::Sealed for TopsContext {}
impl ContextHandle for TopsContext {
    type RawContextT = topsCtx_t;
    fn get_inner(&self) -> Self::RawContextT {
        self.inner
    }
}

unsafe impl Send for TopsContext {}
unsafe impl Sync for TopsContext {}

impl Clone for TopsContext {
    fn clone(&self) -> Self {
        ...
        // is mostly okay
        Self::new(TopsDevice::new(self.device))
        .expect("Failed to clone context")
    }
}


impl ContextTrait for TopsContext {
    ...
    fn new(device: Self::DeviceT) -> DeviceResult<Self::ContextT>
    {
        ...
        unsafe {
            driv::topsDevicePrimaryCtxRetain(inner.as_mut_ptr(), device.as_raw()).to_result()?;
            let inner = inner.assume_init();
            driv::topsCtxSetCurrent(inner);
            Ok(Self::ContextT {
                inner: inner,
    ...
    {
        driv::topsDevicePrimaryCtxReset(device.as_raw()).to_result()
    }
    ...
    {
        unsafe { driv::topsDevicePrimaryCtxSetFlags(self.device, flags.bits()).to_result() }
    }
    ...
    fn get_api_version(&self) -> DeviceResult<Self::ApiVersionT>
    {
        unsafe {
            ...
            driv::topsCtxGetApiVersion(self.inner, &mut api_version as *mut ::std::os::raw::c_int).to_result()?;
            Ok(TopsApi {
    ...
    fn drop(mut ctx: Self::ContextT) -> DropResult<Self::ContextT>
    {
        ...
        unsafe {
            ...
            match driv::topsDevicePrimaryCtxRelease(ctx.device).to_result() {
                Ok(()) => {
                    mem::forget(ctx);
                    Ok(())
                }
...
//     }
// }
pub struct TopsCurrentContext;

impl CurrentContextTrait for TopsCurrentContext {
    type DeviceT = TopsDevice;
    type ContextT = TopsContext;
    ...
    fn get_cache_config() -> DeviceResult<CacheConfig>
    {
        unsafe {
            ...
            driv::topsCtxGetCacheConfig(&mut config as *mut CacheConfig as *mut driv::topsFuncCache_t)
                .to_result()?;
            Ok(config)
        }
    }
    ...
    {
        unsafe {
            let mut device = TopsDevice::new(0);
            driv::topsCtxGetDevice(&mut device.as_raw() as *mut topsDevice_t).to_result()?;
            Ok(device)
        }
    }
    ...
    fn get_flags() -> DeviceResult<ContextFlags>
    {
        unsafe {
            let mut flags = 0u32;
            driv::topsCtxGetFlags(&mut flags as *mut u32).to_result()?;
    ...
    fn get_resource_limit(resource: ResourceLimit) -> DeviceResult<usize>
    {
        unsafe {
            ...
            //TODO
            // driv::topsCtxGetLimit(&mut limit as *mut usize, transmute(resource)).to_result()?;
            Ok(limit)
        }
    }
    ...
    fn get_shared_memory_config() -> DeviceResult<SharedMemoryConfig>
    {
        unsafe {
            ...
            driv::topsCtxGetSharedMemConfig(
                &mut cfg as *mut SharedMemoryConfig as *mut driv::topsSharedMemConfig,
            )
            .to_result()?;
            Ok(cfg)
        }
    ...
    fn get_stream_priority_range() -> DeviceResult<StreamPriorityRange>
    {
        unsafe {
            ...
            driv::topsDeviceGetStreamPriorityRange(
    ...
    {
        unsafe { driv::topsCtxSetCacheConfig(transmute(cfg)).to_result() }
    }
    ...
    fn set_resource_limit(resource: ResourceLimit, limit: usize) -> DeviceResult<()>
    {
        unsafe {
            //TODO
            // driv::topsCtxSetLimit(transmute(resource), limit).to_result()?;
            Ok(())
        }
    }
    ...
    {
        unsafe { driv::topsCtxSetSharedMemConfig(transmute(cfg)).to_result() }
    }
    ...
    fn set_current(c: &Self::ContextT) -> DeviceResult<()>
    {
        ...
        unsafe {
            driv::topsCtxSetCurrent(c.get_inner()).to_result()?;
            Ok(())
        }
    }
    ...
    {
        unsafe {
            driv::topsCtxSynchronize().to_result()?;
            Ok(())
        }
    }
}
