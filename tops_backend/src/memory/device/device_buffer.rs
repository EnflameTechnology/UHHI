...
impl<T: DeviceCopy> DeviceBufferTrait<T> for TopsDeviceBuffer<T> {
    ...
    fn drop_async(self, stream: &Self::StreamT) -> DeviceResult<()>
    {
        ...
        unsafe { TopsMemory::free_async(stream, me.buf) }
    }
    ...
    fn manual_drop(mut dev_buf: &mut Self::DeviceBufferT) -> DropResult<Self::DeviceBufferT>
    {
        ...
        if dev_buf.len > 0 && size_of::<T>() > 0 {
            ...
            let ptr = mem::replace(&mut dev_buf.buf, TopsDevicePointer::null());
            unsafe {
...
impl<T: DeviceCopy + Zeroable> TopsDeviceBuffer<T> {
    ...
    pub fn zeroed(size: usize) -> DeviceResult<TopsDeviceBuffer<T>>
    {
        unsafe {
            ...
            if size_of::<T>() != 0 {
                driv::topsMemsetD8(new_buf.as_device_ptr().as_raw(), 0, size_of::<T>() * size)
                    .to_result()?;
            }
            Ok(new_buf)
        }
...
#[cfg(feature = "bytemuck")]
impl<A: DeviceCopy + Pod> TopsDeviceBuffer<A> {
    ...
    #[cfg_attr(docsrs, doc(cfg(feature = "bytemuck")))]
    pub fn cast<B: Pod + DeviceCopy>(self) -> TopsDeviceBuffer<B> {
    ...
    #[cfg_attr(docsrs, doc(cfg(feature = "bytemuck")))]
    pub fn try_cast<B: Pod + DeviceCopy>(self) -> Result<TopsDeviceBuffer<B>, PodCastError> {
...
impl<T: DeviceCopy> Deref for TopsDeviceBuffer<T> {
    type Target = TopsDeviceSlice<T>;

    fn deref(&self) -> &TopsDeviceSlice<T> {
        unsafe { &*(self as *const _ as *const TopsDeviceSlice<T>) }
    }
}

impl<T: DeviceCopy> DerefMut for TopsDeviceBuffer<T> {
    fn deref_mut(&mut self) -> &mut TopsDeviceSlice<T> {
        unsafe { &mut *(self as *mut _ as *mut TopsDeviceSlice<T>) }
    }
}
...
mod test_device_buffer {
    ...
    fn test_async_copy_to_from_device() {
        ...
        let mut end = [0u64, 0, 0, 0, 0, 0];
        unsafe {
            let buf = TopsDeviceBuffer::from_slice_async(&start, &stream).unwrap();
    ...
    fn test_async_copy_to_d2d_wrong_size() {
        ...
        unsafe {
            let buf = TopsDeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4, 5], &stream).unwrap();
            let mut end = TopsDeviceBuffer::from_slice_async(&[0u64, 1, 2, 3, 4], &stream).unwrap();
            let _ = buf.async_copy_to(&mut end, &stream);
        }
    }
