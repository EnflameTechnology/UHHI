...
#[repr(C)]
pub struct CuDeviceSlice<T: DeviceCopy> {
    pub(crate) ptr: CuDevicePointer<T>,
    len: usize,
}

unsafe impl<T: Send + DeviceCopy> Send for CuDeviceSlice<T> {}
unsafe impl<T: Sync + DeviceCopy> Sync for CuDeviceSlice<T> {}

impl<T: DeviceCopy + Default + Clone> CuDeviceSlice<T> {
...
impl<T: DeviceCopy> CuDeviceSlice<T> {
    ...
    */
    ...
    pub unsafe fn from_raw_parts(ptr: CuDevicePointer<T>, len: usize) -> Self {
        CuDeviceSlice { ptr, len }
    }
    ...
    pub unsafe fn from_raw_parts_mut(ptr: CuDevicePointer<T>, len: usize) -> Self {
        CuDeviceSlice { ptr, len }
    }
}
...
impl<T: DeviceCopy + Pod> DeviceSliceTrait for CuDeviceSlice<T>{
    ...
    fn set_8(&mut self, value: u8) -> DeviceResult<()>{
        ...
        unsafe {
            driv::cuMemsetD8_v2(self.ptr.as_raw(), value, size_of::<T>() * self.len).to_result()
        }
    }
    ...
    fn set_16(&mut self, value: u16) -> DeviceResult<()>{
        ...
        unsafe { driv::cuMemsetD16_v2(self.ptr.as_raw(), value, data_len / 2).to_result() }
    }
    ...
    unsafe fn set_16_async(&mut self, value: u16, stream: &Self::StreamT) -> DeviceResult<()>{
        ...
        driv::cuMemsetD16Async(self.ptr.as_raw(), value, data_len / 2, stream.as_inner())
            .to_result()
    }
    ...
    fn set_32(&mut self, value: u32) -> DeviceResult<()>{
        ...
        unsafe { driv::cuMemsetD32_v2(self.ptr.as_raw(), value, data_len / 4).to_result() }
    }
...
#[cfg(feature = "bytemuck")]
impl<T: DeviceCopy + Zeroable + Pod> CuDeviceSlice<T> {
    // type DevicePointerT = CuDevicePointer<T>;
    // type StreamT = CuStream;
...
pub trait DeviceSliceIndex<T: DeviceCopy> {
    ...
    unsafe fn get_unchecked(self, slice: &CuDeviceSlice<T>) -> CuDeviceSlice<T>;
    fn index(self, slice: &CuDeviceSlice<T>) -> CuDeviceSlice<T>;
}
...
impl<T: DeviceCopy> DeviceSliceIndex<T> for Range<usize> {
    ...
    unsafe fn get_unchecked(self, slice: &CuDeviceSlice<T>) -> CuDeviceSlice<T> {
        CuDeviceSlice::from_raw_parts(slice.as_device_ptr().add(self.start), self.end - self.start)
    }
    fn index(self, slice: &CuDeviceSlice<T>) -> CuDeviceSlice<T> {
...
impl<T: DeviceCopy> CuDeviceSlice<T> {
    pub fn index<Idx: DeviceSliceIndex<T>>(&self, idx: Idx) -> CuDeviceSlice<T> {
        idx.index(self)
    }
}

//CUDA Implementation
// impl<T: DeviceCopy> crate::memory::private::Sealed for CuDeviceSlice<T> {}
impl<T: DeviceCopy, I: AsRef<[T]> + AsMut<[T]> + ?Sized> CopyDestination<I> for CuDeviceSlice<T> {
    fn copy_from(&mut self, val: &I) -> DeviceResult<()> {
        ...
        if size != 0 {
            unsafe {
                driv::cuMemcpyHtoD_v2(self.ptr.as_raw(), val.as_ptr() as *const c_void, size)
                    .to_result()?
            }
        }
        Ok(())
...
impl<T: DeviceCopy> CopyDestination<CuDeviceSlice<T>> for CuDeviceSlice<T> {
    fn copy_from(&mut self, val: &CuDeviceSlice<T>) -> DeviceResult<()> {
        assert!(
            self.len() == val.len(),
            "destination and source slices have different lengths"
        );
        ...
        if size != 0 {
            unsafe {
                driv::cuMemcpyDtoD_v2(self.ptr.as_raw(), val.as_device_ptr().as_raw(), size)
                    .to_result()?
            }
        }
        Ok(())
    ...
    fn copy_to(&self, val: &mut CuDeviceSlice<T>) -> DeviceResult<()> {
        assert!(
            self.len() == val.len(),
            "destination and source slices have different lengths"
        );
...
impl<T: DeviceCopy> CopyDestination<CuDeviceBuffer<T>> for CuDeviceSlice<T> {
    fn copy_from(&mut self, val: &CuDeviceBuffer<T>) -> DeviceResult<()> {
        self.copy_from(val as &CuDeviceSlice<T>)
    }

    fn copy_to(&self, val: &mut CuDeviceBuffer<T>) -> DeviceResult<()> {
        self.copy_to(val as &mut CuDeviceSlice<T>)
    }
}
