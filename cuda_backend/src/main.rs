...
fn network_test() -> DeviceResult<()> {
    ...
    let layers = vec![
        Layer::<f32> {
            op: "matmul",
            weight: Some(CuDeviceBuffer::from_slice(&[0.01f32; N * N])?),
            input_size: (N, N),
            output_size: (N, N),
            out_ref: None,
        ...
        Layer::<f32> {
            op: "matmul",
            weight: Some(CuDeviceBuffer::from_slice(&[0.02f32; N * N])?),
            input_size: (N, N),
            output_size: (N, N),
            out_ref: None,
        }, //weight is N x N matric for next layer
        ...
        Layer::<f32> {
            op: "matmul",
            weight: Some(CuDeviceBuffer::from_slice(&[0.5f32; K * K])?),
            input_size: (N, N),
            output_size: (N, N),
            out_ref: None,
        ...
        Layer::<f32> {
            op: "convolution",
            weight: Some(CuDeviceBuffer::from_slice(
                &[0.2f32; (N - K + 1) * (N - K + 1)],
            )?),
            input_size: (N, N),
...
fn test() -> DeviceResult<()> {
    ...
    let matA = CuDeviceBuffer::from_slice(&[
        1.0f32, 2.0f32, 3.0f32, 4.0f32, 5.0f32, 6.0f32, 1.0f32, 2.0f32, 3.0f32,
    ])?;
    let matB = CuDeviceBuffer::from_slice(&[
        4.0f32, 5.0f32, 1.0f32, 2.0f32, 3.0f32, 4.0f32, 5.0f32, 1.0f32, 2.0f32,
    ])?;
