...
pub enum ResourceLimit {
    ...
    /// `DeviceSynchronize()` to wait on child grid launches to complete.
...
bitflags::bitflags! {
    /// Bit flags for initializing the Device context.
    ///
    /// If you're not sure which flags to use, `MAP_HOST | SCHED_AUTO` is a good default.
    pub struct ContextFlags: u32 {
        /// Instructs Device to actively spin when waiting for results from the GPU. This can decrease
        /// latency when waiting for the GPU, but may lower the performance of other CPU threads
        /// if they are performing work in parallel with the Device thread.
        const SCHED_SPIN = 0x01;

        /// Instructs Device to yield its thread when waiting for results from the GPU. This can
        ...
        /// Instructs Device to block the CPU thread on a synchronization primitive when waiting for
        /// the GPU to finish work.
        const SCHED_BLOCKING_SYNC = 0x04;

        /// Instructs Device to automatically choose whether to yield to other OS threads while waiting
        /// for the GPU, or to spin the OS thread. This is the default.
        const SCHED_AUTO = 0x00;

        /// Instructs Device to support mapped pinned allocations. This flag must be set in order to
        /// use page-locked memory (see [LockedBuffer](../memory/struct.LockedBuffer.html])).
        const MAP_HOST = 0x08;

        /// Instruct Device not to reduce local memory after resizing local memory for a kernel. This
...
pub trait CurrentContextTrait {
    ...
    // fn set_current<C: ContextHandle>(c: &C) -> DeviceResult<()>;
